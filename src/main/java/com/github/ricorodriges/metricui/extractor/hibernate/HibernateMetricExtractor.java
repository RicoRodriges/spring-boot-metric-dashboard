package com.github.ricorodriges.metricui.extractor.hibernate;
...
/**
 * @see org.hibernate.stat.HibernateMetrics
 */
@UtilityClass
public final class HibernateMetricExtractor {
    ...
    private static QueryMetricResult extractNaturalIdQueries(Collection<MeterData> meters, String manager) {
        Duration max = findMetersByName(meters, "hibernate.query.natural.id.executions.max")
                .filter(m -> manager.equals(getEntityManagerFactory(m)))
                .findAny()
                .flatMap(ExtractorUtils::getFirstValueAsDuration)
        ...
        return new QueryMetricResult(hits, misses, puts, executions, max);
    }

    private static QueryMetricResult extractQueries(Collection<MeterData> meters, String manager) {
        Duration max = findMetersByName(meters, "hibernate.query.executions.max")
                .filter(m -> manager.equals(getEntityManagerFactory(m)))
                .findAny()
                .flatMap(ExtractorUtils::getFirstValueAsDuration)
                .orElse(Duration.ZERO);
